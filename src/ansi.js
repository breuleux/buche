...
export class AnsiParser {
  ...
  _apply(codes, i) {
    const c = codes[i];
    if      (c === 0)                  { this.fg = this.bg = null; this.bold = this.italic = this.underline = false; }
    ...
    else if (c >= 30 && c <= 37)       { this.fg = C16[c - 30]; }
    else if (c === 38 && codes[i+1] === 5) { this.fg = c256(codes[i+2]); return 2; }
    else if (c === 38 && codes[i+1] === 2) { this.fg = `rgb(${codes[i+2]},${codes[i+3]},${codes[i+4]})`; return 4; }
    else if (c === 39)                 { this.fg = null; }
    else if (c >= 40 && c <= 47)       { this.bg = C16[c - 40]; }
    else if (c === 48 && codes[i+1] === 5) { this.bg = c256(codes[i+2]); return 2; }
    else if (c === 48 && codes[i+1] === 2) { this.bg = `rgb(${codes[i+2]},${codes[i+3]},${codes[i+4]})`; return 4; }
    else if (c === 49)                 { this.bg = null; }
    else if (c >= 90 && c <= 97)       { this.fg = C16[c - 90 + 8]; }
    else if (c >= 100 && c <= 107)     { this.bg = C16[c - 100 + 8]; }
    return 0;
  }
  ...
  parse(text, streamClass) {
    ...
    for (const m of src.matchAll(SGR_RE)) {
      if (m.index > last) nodes.push(this._makeSpan(src.slice(last, m.index), streamClass));
      last = m.index + m[0].length;
      const codes = m[1] === '' ? [0] : m[1].split(';').map(Number);
      for (let i = 0; i < codes.length; i++) i += this._apply(codes, i);
    }

    if (last < src.length) nodes.push(this._makeSpan(src.slice(last), streamClass));
    return nodes;
  }
}
