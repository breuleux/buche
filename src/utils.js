...
export function html(strings, ...values) {
  ...
  const commentMarkers = values.map(
    (_, i) => `<!--__HTML_PLACEHOLDER_${i}__-->`,
  );
...
export function toggle(strings, ...values) {
  ...
  if (!togglerElement) {
    throw new Error(
      'toggle`` requires an element with the "toggler" attribute',
    );
  }
  if (!toggledElement) {
    throw new Error(
      'toggle`` requires an element with the "toggled" attribute',
    );
  }
