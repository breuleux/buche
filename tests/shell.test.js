...
async function run(...items) {
    ...
    const results = [];
    async function* stream() { for (const item of items) yield item; }
...
describe("run", () => {
    test("emits new and close", async () => {
        ...
        expect(out[out.length - 1]).toMatchObject({ type: "close", cell_id: "p1", return_code: 0 });
    });
    ...
    test("captures stdout", async () => {
        const out = await run(
            { type: "run", command: "echo", args: ["hello"], cell_id: "p1" },
        );
    ...
    test("captures stderr", async () => {
        const out = await run(
            { type: "run", command: "sh", args: ["-c", "echo err >&2"], cell_id: "p1" },
        );
    ...
    test("non-zero return code", async () => {
        const out = await run(
            { type: "run", command: "false", cell_id: "p1" },
        );
    ...
    test("ENOENT emits error", async () => {
        const out = await run(
            { type: "run", command: "no_such_command_xyz", cell_id: "p1" },
        );
        const error = out.find(e => e.type === "error");
...
describe("parse", () => {
    test("runs the parsed command", async () => {
        const out = await run(
            { type: "parse", text: "echo hello", cell_id: "p1" },
        );
        expect(out[0]).toMatchObject({ type: "new", cell_id: "p1" });
        const stdout = out.filter(e => e.type === "send" && e.data.stream === "stdout");
        expect(stdout[0].data.text).toContain("hello");
    ...
    test("uses provided cell_id", async () => {
        const out = await run(
            { type: "parse", text: "true", cell_id: "my-id" },
        );
    ...
    test("generates cell_id when null", async () => {
        const out = await run(
            { type: "parse", text: "true", cell_id: null },
        );
        expect(out[0].type).toBe("new");
        expect(typeof out[0].cell_id).toBe("string");
    });
    ...
    test("handles multiple args", async () => {
        const out = await run(
            { type: "parse", text: "echo foo bar baz", cell_id: "p1" },
        );
        const stdout = out.filter(e => e.type === "send" && e.data.stream === "stdout");
